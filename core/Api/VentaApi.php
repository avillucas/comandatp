...
class VentaApi extends ApiUsable
{

    public function TraerTodos($request, $response, $args)
    {        
        $todos = VentaEntidadDao::traerTodosConRelaciones();
        return $response->withJson($todos, 200);
    }

    public function BorrarUno($request, $response, $args)
    {        
        $producto = VentaEntidadDao::traerUno($args['id']);
        VentaEntidadDao::eliminar($producto);
